...
$(function () {
  ...
  $("#nav_mo > ul > li").click(function () {
    ...
    if ($(this).children(".nav_mo_sub").is(":visible")) {
      ...
      menu_button.animate(
        { top: nav_height2 - nav_mo_sub_height + "px" },
        "fast"
      );
  ...
  $(window).load(function () {
    ...
    slide_control.css({
      top: stage_height - 30 + "px",
      left: (stage_width - slide_control_width) / 2 + "px",
    });
    modal_gallery();
  });
  ...
  for (var i = 1; i <= last; i++) {
    if (i == 1) {
      $("#slide_control #number_last").append(
        '<span id="photo_' + i + '"><img src="images/active.png"/></span>'
      );
    } else {
      $("#slide_control #number_last").append(
        '<span id="photo_' + i + '"><img src="images/active.png"/></span>'
  ...
  function slide_start() {
    slide_set = setInterval(function () {
      if (number == last) {
        $("#stage ul li:nth-child(" + number + ")").animate(
          { opacity: 0 },
          change_photo
        );
        ...
        $("#stage ul li:nth-child(" + number + ")")
          .animate({ opacity: 0 }, change_photo)
          .next("li")
          .animate({ opacity: 1 }, change_photo);
        number++;
  ...
  $("[id^='photo_']").on("click", function () {
    ...
    slide_stop();
    $("#stage ul li:not(:nth-child(" + id_slice + "))").animate(
      { opacity: 0 },
      change_photo_click
    );
    $("#stage ul li:nth-child(" + id_slice + ")").animate(
      { opacity: 1 },
      change_photo_click
    );
    number = id_slice;
  ...
  $("#prev_button").click(function () {
    ...
    if (number == 1) {
      ...
      $("#stage ul li:nth-child(" + last + ")").animate(
        { opacity: 1 },
        change_photo_click
      );
      number = last;
      ...
      $("#stage ul li:nth-child(" + number + ")")
        .animate({ opacity: 0 }, change_photo_click)
        .prev("li")
        .animate({ opacity: 1 }, change_photo_click);
      number--;
    }
    $("#photo_" + number).animate({ opacity: 1 }, change_photo_click);
    for (var i = 1; i <= last; i++) {
  ...
  if (modal_state == 1) {
    ...
    modal_dt_img.on("click", function () {
      ...
      modal_window
        .html(dd_html)
        .css({
          width: window_width - modal_width_minus + "px",
          height: window_height - modal_height_minus + "px",
          left: 10 + "px",
          top: 10 + "px",
        })
        .fadeIn("normal", function () {
          ...
          if (window_height - modal_height_minus - modal_img_height < 0) {
            ...
            window_width - modal_width_minus - modal_img_width > 0 &&
            window_height - modal_height_minus - modal_img_height > 0
          ) {
            $("#modal_window img")
              .css(
                "margin-top",
                (window_height - modal_height_minus - modal_img_height) / 2 +
                  "px"
              )
              .fadeIn();
          } else {
          ...
          modal_prev_button
            .css({ top: window_height / 2 - 50 + "px", left: "5px" })
            .fadeIn();
          modal_next_button
            .css({
              top: window_height / 2 - 50 + "px",
              left: window_width - 105 + "px",
            })
            .fadeIn();
          modal_close_button
            .css({ top: "15px", left: window_width - 35 + "px" })
            .fadeIn();
            ...
            if (dt_index == 0) {
              ...
              modal_window.fadeOut("normal", function () {
                modal_window.html(dd_html).fadeIn("normal", function () {
                  ...
                  if (
                    window_height - modal_height_minus - modal_img_height <
                    0
                  ) {
                    $("#modal_window img")
                      .css({ width: "auto", height: "100%" })
